...
def validate_jalali_date(date_str):
    try:
        ...
        JalaliDate(year, month, day)  # Validate by creating JalaliDate object
        return True
...
class FilterThread(QThread):
    ...
    def run(self):
        ...
        if self.filters['device'] != "All Devices":
            filtered_df = filtered_df.query("folder_name.str.contains(@self.filters['device'], regex=False)")
        self.progress_updated.emit(40)
        if self.filters['element'] != "All Elements":
            base_element = self.filters['element']
            filtered_df = filtered_df[filtered_df['element'].str.startswith(base_element + ' ') | (filtered_df['element'] == base_element)]
        self.progress_updated.emit(60)
        if self.filters['crm'] != "All CRM IDs":
            filtered_df = filtered_df.query("norm_crm_id == @self.filters['crm']")
        
        if self.filters['from_date']:
            try:
                filtered_df = filtered_df[filtered_df['date'] >= self.filters['from_date']]
            except Exception as e:
                logging.error(f"Invalid from_date: {self.filters['from_date']}, {str(e)}")
        
        if self.filters['to_date']:
            try:
                filtered_df = filtered_df[filtered_df['date'] <= self.filters['to_date']]
            except Exception as e:
                logging.error(f"Invalid to_date: {self.filters['to_date']}, {str(e)}")
...
class CRMDataVisualizer(QMainWindow):
    def __init__(self):
        ...
        self.device_combo.addItem("Devices")
        self.element_combo.addItem("Elements")
        self.crm_combo.addItem("CRM IDs")
        ...
        # self.filter_layout.addStretch()  # Add stretch after buttons to center them
    ...
    def populate_filters(self):
        ...
        devices = sorted(set(self.extract_device_name(folder) for folder in self.df['folder_name']))
        elements = sorted(set(el.split()[0] for el in self.df['element'].unique()))
    ...
    def get_verification_value(self, crm_id, element):
        ...
        try:
            logging.debug(f"Connecting to database: {self.ver_db_path}")
            conn = sqlite3.connect(self.ver_db_path)

            cursor = conn.cursor()
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
            tables = [row[0] for row in cursor.fetchall()]
            logging.debug(f"Available tables in {self.ver_db_path}: {tables}")

            table_name = "oreas_hs j" if re.match(r'(?i)oreas', crm_id) else "pivot_crm"
            if table_name not in tables:
                logging.error(f"Table {table_name} does not exist in database {self.ver_db_path}")
                conn.close()
                QMessageBox.critical(self, "Error", f"Table {table_name} does not exist in database.")
                self.verification_cache[cache_key] = None
                return None

            logging.debug(f"Querying table {table_name} for CRM {crm_id}, Element {element}")

            cursor.execute(f"PRAGMA table_info({table_name})")
            cols = [x[1] for x in cursor.fetchall()]
            logging.debug(f"Columns in {table_name}: {cols}")
            if 'CRM ID' not in cols:
                logging.error(f"Column 'CRM ID' not found in {table_name}")
                conn.close()
                QMessageBox.critical(self, "Error", f"Column 'CRM ID' not found in table {table_name}.")
                self.verification_cache[cache_key] = None
                return None

            element_base = element.split()[0] if ' ' in element else element
            logging.debug(f"Base element (without wavelength): {element_base}")
            if element not in cols and element_base not in cols:
                logging.error(f"Element {element} or {element_base} not found in {table_name} columns: {cols}")
                conn.close()
                QMessageBox.critical(self, "Error", f"Element {element} not found in table {table_name}.")
                self.verification_cache[cache_key] = None
                return None
            ...
            logging.debug(f"Query result for CRM ID LIKE '%{crm_id_part}%' in {table_name}: {crm_data}")

            if not crm_data:
                logging.warning(f"No CRM data found for {crm_id} or partial matches in {table_name}")
                conn.close()
                self.verification_cache[cache_key] = None
                return None
            ...
            for row in crm_data:
                ...
                logging.debug(f"Checking label: {label} against CRM ID: {crm_id}")
                if label.find(crm_id_part.upper()) != -1:
                    ...
                    if value is not None and not pd.isna(value):
                        ...
                        logging.debug(f"No valid value for {target_element} in row: {row_dict}")

            logging.warning(f"No matching CRM ID {crm_id} with valid value for {target_element} in {table_name}")
            self.verification_cache[cache_key] = None
            return None
        except Exception as e:
            logging.error(f"Error querying {table_name} for CRM {crm_id}, Element {element}: {str(e)}")
    ...
    def plot_data(self):
        ...
        # Get the percentage for control limits
        percentage = 10.0  # Default value
        ...
        crm_ids = [crm for crm in ([current_crm] if current_crm != "All CRM IDs" else filtered_df['norm_crm_id'].unique())]
        ...
        for idx, crm_id in enumerate(crm_ids):
            ...
            if current_element != "All Elements" and self.best_wl_check.isChecked():
                ...
                if ver_value is not None:
                    ...
                    logging.warning(f"No verification value for {crm_id}, {current_element}, skipping best wl selection")

            if crm_df.empty:
                logging.debug(f"No data for CRM ID {crm_id} after filtering")
                continue
            ...
            logging.debug(f"Indices length: {len(indices)}, Values length: {len(values)}")

            if len(indices) > 1000:
                sample_indices = np.linspace(0, len(indices) - 1, 1000, dtype=int)
                indices = indices[sample_indices]
                values = values[sample_indices]
                date_labels = [date_labels[i] for i in sample_indices]
                crm_df = crm_df.iloc[sample_indices]
                logging.debug(f"Sampled down to {len(indices)} points for CRM ID {crm_id}")
            ...
            logging.debug(f"Plotted data for CRM ID {crm_id}, {len(crm_df)} points")

            if current_element != "All Elements" and current_crm != "All CRM IDs":
                ver_value = self.get_verification_value(crm_id, current_element)
                logging.debug(f"Verification value for CRM {crm_id}, Element {current_element}: {ver_value}")
                if ver_value is not None and not pd.isna(ver_value):
                    x_range = [0, max(indices) if indices.size > 0 else 0]
                    delta = ver_value * (percentage / 100) / 3
                    self.plot_widget.plot(x_range, [ver_value * (1 - percentage / 100), ver_value * (1 - percentage / 100)], pen=mkPen('b', width=2, style=Qt.PenStyle.DotLine), name="LCL")
                    self.plot_widget.plot(x_range, [ver_value - 2 * delta, ver_value - 2 * delta], pen=mkPen('c', width=1, style=Qt.PenStyle.DotLine), name="-2LS")
                    self.plot_widget.plot(x_range, [ver_value - delta, ver_value - delta], pen=mkPen('c', width=1, style=Qt.PenStyle.DotLine), name="-1LS")
                    self.plot_widget.plot(x_range, [ver_value, ver_value], pen=mkPen('k', width=3, style=Qt.PenStyle.DashLine), name=f"Ref Value ({ver_value:.3f})")
                    self.plot_widget.plot(x_range, [ver_value + delta, ver_value + delta], pen=mkPen('m', width=1, style=Qt.PenStyle.DotLine), name="1LS")
                    self.plot_widget.plot(x_range, [ver_value + 2 * delta, ver_value + 2 * delta], pen=mkPen('m', width=1, style=Qt.PenStyle.DotLine), name="2LS")
                    self.plot_widget.plot(x_range, [ver_value * (1 + percentage / 100), ver_value * (1 + percentage / 100)], pen=mkPen('r', width=2, style=Qt.PenStyle.DotLine), name="UCL")
                    logging.info(f"Plotted verification lines for CRM {crm_id}, Element {current_element} with {percentage}% range")
                else:
                    logging.warning(f"Skipping verification lines for CRM {crm_id}, Element {current_element} due to None or NaN value")

        self.plot_df_cache = plot_df
        self.update_table(plot_df)
        self.plot_widget.showGrid(x=True, y=True)
        self.status_label.setText(f"Plotted {len(plot_df)} records")
        logging.info(f"Plotted {len(plot_df)} records")
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            for plot_item, crm_df, indices, date_labels in self.plot_data_items:
                for i, (idx, value, date) in enumerate(zip(indices, crm_df['value'], date_labels)):
                    ...
                    if dist < closest_dist and dist < 0.5:
                        closest_dist = dist
            ...
            if closest_info:
                ...
                logging.debug(f"Clicked on point with info: {closest_info}")
            else:
