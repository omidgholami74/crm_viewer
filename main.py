...
class CRMDataVisualizer(QMainWindow):
    def __init__(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(["ID", "CRM ID", "Solution Label", "Element", "Value","Refrence", "Blank Value", "File Name", "Date","Ref Proximity %"])
    ...
    def update_table(self, filtered_crm_df=None, filtered_blank_df=None):
        ...
        for i, row in combined_df.iterrows():
            ...
            self.table_widget.setItem(i, 5, QTableWidgetItem(str(ver_value)))
            self.table_widget.setItem(i, 6, QTableWidgetItem(f"{row.get('blank_value', ''):.6f}" if pd.notna(row.get('blank_value')) else ""))
            self.table_widget.setItem(i, 7, QTableWidgetItem(str(row['file_name'])))
            self.table_widget.setItem(i, 8, QTableWidgetItem(str(row['date']) if pd.notna(row['date']) else ""))
            self.table_widget.setItem(i, 9, QTableWidgetItem(f"{row['ref_proximity']:.2f}%" if pd.notna(row['ref_proximity']) else ""))
